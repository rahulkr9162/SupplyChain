...
public class ProductDetails {
    ...
    public Pane getAllProducts() {
        ...
        table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY); // to remove extra column
        table.getSelectionModel().setSelectionMode(SelectionMode.SINGLE);
        table.getColumns().addAll(idCol, nameCol, priceCol);
        table.setPrefSize(SupplyChain.width-10,SupplyChain.height-100);

        productTable = table;
        Pane tpane = new Pane();
        tpane.getChildren().add(table);
        tpane.setPrefSize(SupplyChain.width,SupplyChain.height);
        tpane.setTranslateY(30);
        return tpane;
    }
